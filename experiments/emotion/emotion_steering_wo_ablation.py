...
SAVE_MODEL_PATH="resources/checkpoints/SLiM_emotions_wo_gate_norm_residue"

def one_hot_encode_emotions(emotion, num_classes=NUM_STATES):
...
def prepare_dataset(max_sequence_length, path='resources/datasets/emotions_500.pth'):
    tokenizer = AutoTokenizer.from_pretrained("gpt2")
    tokenizer.pad_token = tokenizer.eos_token
    if not os.path.exists(path):
        dataset = load_dataset('ma2za/many_emotions', 'raw') # [ 'anger', 'fear', 'joy', 'love', 'sadness' ]
        texts = []
    ...
    def create_film_hook(self, layer_idx):
        def hook(module, input, output):
            if self.current_state_embed is not None:
                ...
                steered_output = output[0] * scale + shift
        ...
        for i, (input_seq, target_seq, state) in enumerate(dataloader):
            ...
            if (i + 1) % accumulation_steps == 0:
                ...
                if step % 500 == 0 or step == max_steps:
                    ...
                    print("=============")                    
                    print("Anger: ", generate_text(model, tokenizer, "I feel", max_length=40, state_tensor=torch.FloatTensor([1,0,0,0,0]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")                    
                    print("fear:", generate_text(model, tokenizer, "I feel", max_length=40, state_tensor=torch.FloatTensor([0,1,0,0,0]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")                    
                    print("joy: ", generate_text(model, tokenizer, "I feel", max_length=40, state_tensor=torch.FloatTensor([0,0,1,0,0]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")                    
                    print("love: ", generate_text(model, tokenizer, "I feel", max_length=40, state_tensor=torch.FloatTensor([0,0,0,1,0]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")                    
                    print("sadness :", generate_text(model, tokenizer, "I feel", max_length=40, state_tensor=torch.FloatTensor([0,0,0,0,1]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============\n")         
                    model.train()           
