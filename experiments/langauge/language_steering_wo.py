...
SAVE_MODEL_PATH="resources/checkpoints/SLiM_langauge_wo"
...
def prepare_dataset(max_sequence_length, path='resources/datasets/language_500.pth'):
        ...
        for i, (input_seq, target_seq, state) in enumerate(dataloader):
            ...
            if (i + 1) % accumulation_steps == 0:
                ...
                if step % 500 == 0 or step == max_steps:
                    model.eval()  # Set to eval mode for text generation
                    print(f"Step {step} loss: {(loss.item()):.3f}")
                    print("\n=============")                    
                    print("Baseline: ", generate_text(model, tokenizer, "I feel", max_length=40, state_tensor=None), '\n')               
                    print("german: ", generate_text(model, tokenizer, "I feel", max_length=40, state_tensor=torch.FloatTensor([0, 1]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")                                     
                    print("english: ", generate_text(model, tokenizer, "I feel", max_length=40, state_tensor=torch.FloatTensor([1, 0]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============\n")    
                    model.train()
