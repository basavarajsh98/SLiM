...
SAVE_MODEL_PATH="resources/checkpoints/SLiM_multi_state"
...
def randomize_rating(rating):
    ...
    if rating == 0:
        return round(uniform(0, 0.19), 2)  # Random value between 0 and 0.5 for ratings 1-2
    if rating == 1:
        return round(uniform(0.2, 0.39), 2)  # Random value between 0 and 0.5 for ratings 1-2
    elif rating == 2:
        return round(uniform(0.4, 0.59), 2)  # Random value between 0 and 0.5 for ratings 1-2
    elif rating == 3:
        return round(uniform(0.6, 0.79), 2)  # Random value between 0 and 0.5 for ratings 1-2
    elif rating == 4:
        return round(uniform(0.8, 1), 2)  # Random value between 0 and 0.5 for ratings 1-2
...
def balance_dataset(texts, min_samples_per_bin=10000):
    ...
    bins = {topic: {rating: [] for rating in range(5)} for topic in TOPIC_TO_INDEX.values()}
    ...
    for topic, topic_bins in bins.items():
        for rating, bin_samples in topic_bins.items():
            ...
            balanced_texts.extend([(text, [label[0], randomize_rating(rating)]) for text, label in bin_samples])
...
def prepare_dataset(max_sequence_length, path='resources/datasets/multi_state.pth'):
    tokenizer = AutoTokenizer.from_pretrained("gpt2")
    tokenizer.pad_token = tokenizer.eos_token
    if not os.path.exists(path):
        MAX_SAMPLES_PER_TOPIC = 10000

        dataset = load_dataset("csv", data_files="resources/datasets/train.csv")    
        texts = []
        topic_counts = defaultdict(int)

        # Prepare texts and labels with topic and randomized rating
        print("Started preparing data...")
        for item in dataset['train']:
            topic = item['product_category']
            topic_index = TOPIC_TO_INDEX.get(topic)
            if topic_index is not None:
                rating = item.get('stars')  # Handle case if 'review/score' field is missing
                lang = item.get('language')

                # Check conditions for language and topic limit
                if lang in ['en', 'de'] and topic_counts[topic] < MAX_SAMPLES_PER_TOPIC:
                    texts.append((item['review_body'], [LANG_TO_INDEX.get(lang), topic_index, rating]))
        ...
        for i, (input_seq, target_seq, state) in enumerate(dataloader):
            ...
            if (i + 1) % accumulation_steps == 0:
                ...
                if step % 500 == 0 or step == max_steps:
                    ...
                    print("=============")  
                    print("book German", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([1,0,3]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============") 
                    print("book English ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,0,3]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")                          
                    print("book bad ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,0,1]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")   
                    print("book good ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,0,5]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")    

                    print("electronics German", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([1,1,3]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============") 
                    print("electronics English ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,1,3]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")                          
                    print("electronics bad ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,1,1]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")   
                    print("electronics good ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,1,5]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")  

                    print("apparel German", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([1,2,3]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============") 
                    print("apparel English ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,2,3]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")                          
                    print("apparel bad ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,2,1]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")   
                    print("apparel good ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,2,5]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")    

                    print("beauty German", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([1,3,3]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============") 
                    print("beauty English ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,3,3]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")                          
                    print("beauty bad ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,3,1]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============")   
                    print("beauty good ", generate_text(model, tokenizer, "I think", max_length=40, state_tensor=torch.FloatTensor([0,3,5]).unsqueeze(0).to(device)).strip(), '\n')
                    print("=============\n")      
                    model.train()                  
