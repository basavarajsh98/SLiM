...
def evaluate_combined_states_with_metrics(model, tokenizer, combined_states, topic_embeddings, num_generations=100):
    steered_results = []
    unsteered_results = []
    similarity_results = []
    ...
    for state in combined_states:
        ...
        generated_texts = generate_text(model=model, tokenizer=tokenizer, prompt="I feel", 
                                        state_tensor=state_tensor, num_generations=num_generations)
        
        # Generate texts without steering (model's natural output)
        unconditioned_texts = generate_text(model=model, tokenizer=tokenizer, prompt="I feel", 
                                            state_tensor=None, num_generations=num_generations)
        ...
        topic_similarity_scores = [compute_similarity(text, topic_embeddings[topic]) for text in generated_texts]
        ...
        unconditioned_similarity_scores = [compute_similarity(text, topic_embeddings[topic]) for text in unconditioned_texts]

        # True labels for target state
        true_lang_label = [1 if lang == classification["label"] else 0 for classification in lang_classifications]
        true_rating_label = [1 if int(rating) in [4, 5] else 0 for _ in range(num_generations)]

        # Predicted labels (binarized) for steered
        pred_lang_label = [1 if classification["label"] == lang else 0 for classification in lang_classifications]
        if rating in ('5', '4'):
            pred_rating_label = [1 if 'positive' in classification["label"].lower() else 0 for classification in senti_classifications]
        if rating in ('1', '2'):
            pred_rating_label = [1 if 'negative' in classification["label"].lower() else 0 for classification in senti_classifications]

        # Predicted labels for unsteered
        pred_unsteered_lang_label = [1 if classification["label"] == lang else 0 for classification in unsteered_lang_classifications]
        if rating in ('5', '4'):
            pred_unsteered_rating_label = [1 if 'positive' in classification["label"].lower() else 0 for classification in unsteered_senti_classifications]
        if rating in ('1', '2'):
            pred_unsteered_rating_label = [1 if 'negative' in classification["label"].lower() else 0 for classification in unsteered_senti_classifications]
        ...
        unsteered_lang_metrics = calculate_metrics(true_lang_label, pred_unsteered_lang_label)
        unsteered_rating_metrics = calculate_metrics(true_rating_label, pred_unsteered_rating_label)
...
# # Example usage:
# evaluate_combined_states_with_metrics(model, tokenizer, combined_states, avg_topic_embedding)
...
# ll the function with combined states
    combined_states = [
        "book_en_1", "book_en_5", "electronics_en_1", "electronics_en_5",
        "clothing_en_1", "clothing_en_5", "beauty_en_1", "beauty_en_5",
        "book_de_1", "book_de_5", "electronics_de_1", "electronics_de_5",
        "clothing_de_1", "clothing_de_5", "beauty_de_1", "beauty_de_5",

        "book_en_2", "book_en_4", "electronics_en_2", "electronics_en_4",
        "clothing_en_2", "clothing_en_4", "beauty_en_2", "beauty_en_4",
        "book_de_2", "book_de_4", "electronics_de_2", "electronics_de_4",
        "clothing_de_2", "clothing_de_4", "beauty_de_2", "beauty_de_4",
    ]
    evaluate_combined_states_with_metrics(model, tokenizer, combined_states, avg_topic_embedding)
