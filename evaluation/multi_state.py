...
def get_average_topic_embedding():
    import os
    if os.path.exists("/home/qequ54zi/thesis/submission/SLiM/average.embedding.pth"):
        return torch.load("/home/qequ54zi/thesis/submission/SLiM/average.embedding.pth")
    else:
        topics = [
...
def get_distilroberta_lang_classifier():
    ...
    classifier = pipeline("text-classification", model="papluca/xlm-roberta-base-language-detection", return_all_scores=False)
    return classifier
...
def evaluate_lang_generations_with_auc(model, tokenizer, state_pairs, num_generations=100):
    # Initialize lists to store all true labels and predicted scores
    all_true_labels = []
    all_predicted_scores = {state.split('_')[-1]: [] for state in [s for pair in state_pairs for s in pair]}
    ...
    for states in state_pairs:
        for state in states:
            ...
            for classification in classifications:
                true_label = [1 if state.split('_')[-1] == target_state.split('_')[-1] else 0 for target_state in states]
                all_true_labels.append(true_label)

                for target_state in states:
                    target_label = target_state.split('_')[-1]
                    score = classification['score'] if classification['label'] == target_label else 0
                    all_predicted_scores[target_label].append(score)
            # Calculate AUC for the current state
            auc = roc_auc_score(true_label, [all_predicted_scores[state.split('_')[-1]][i] for i in range(len(true_label))])
    ...
    plot_roc_curves(all_true_labels, all_predicted_scores, "Language Detection ROC Curves", "lang")
...
def plot_roc_curves(true_labels, predicted_scores, title, plot_type):
    ...
    for label, scores in predicted_scores.items():
        fpr, tpr, _ = roc_curve(true_labels[:, list(predicted_scores.keys()).index(label)], scores)
        auc = roc_auc_score(true_labels[:, list(predicted_scores.keys()).index(label)], scores)
...
def evaluate_senti_generations_with_auc(model, tokenizer, state_pairs, num_generations=100):
        ...
        for state in states:
            ...
            for classification in classifications:
                ...
                for target in states:
                    ...
                    score = classification['score'] if classification['label'].lower() == target_label.split('_')[0] else 0
                    all_predicted_scores[target].append(score)
                    predicted_label = 1 if classification['label'].lower() == target_label.split('_')[0] else 0
...
def aggregate_topic_sentiment_metrics(all_senti_states, model, tokenizer, num_generations=100):
    # Evaluate and collect metrics for each topic and sentiment type
    all_metrics = evaluate_senti_generations_with_auc(model, tokenizer, all_senti_states, num_generations)
...
def unsteered_evaluate_senti_generations_with_auc(model, tokenizer, state_pairs, num_generations=100):
    # Initialize dictionary to store metrics per topic and sentiment type
    metrics_by_topic_sentiment = {}
...
def unsteered_aggregate_topic_sentiment_metrics(all_senti_states, model, tokenizer, num_generations=100):
    # Evaluate and collect metrics for each topic and sentiment type
    all_metrics = unsteered_evaluate_senti_generations_with_auc(model, tokenizer, all_senti_states, num_generations)
...
def evaluate_and_aggregate_metrics(model, tokenizer, all_senti_states, num_generations=100, num_runs=5):
    ...
    def evaluate_generations(model, tokenizer, states, is_steered, num_generations):
        ...
        for state_group in states:
            ...
            for state in state_group:
                ...
                # Generate texts
                state_tensor = get_state_tensor(state, device) if is_steered else None
                generated_texts = generate_text(model, tokenizer, prompt="I feel", state_tensor=state_tensor, num_generations=num_generations)
                classifications = senti_classifier(generated_texts)
    ...
    for _ in range(num_runs):
        steered_metrics_runs.append(evaluate_generations(model, tokenizer, all_senti_states, is_steered=True, num_generations=num_generations))
        unsteered_metrics_runs.append(evaluate_generations(model, tokenizer, all_senti_states, is_steered=False, num_generations=num_generations))
...
if __name__ == "__main__":
    ...
    # averaged_results = aggregate_topic_sentiment_metrics(all_senti_states, model, tokenizer)
    ...
    #         print(f"  {senti.capitalize()}: Accuracy: {metrics['accuracy_avg']:.2f} +- {metrics['accuracy_std']:.2f}, F1: {metrics['f1_avg']:.2f} +- {metrics['f1_std']:.2f}, AUC: {metrics['auc_avg']:.2f}")

    # Aggregate metrics by topic and sentiment
    # unsteered_averaged_results = unsteered_aggregate_topic_sentiment_metrics(all_senti_states, model, tokenizer)
    ...
    #     print(f"\nTopic: {topic.capitalize()}")
    #     for senti, metrics in sentiments.items():
    #         print(f"  {senti.capitalize()}: Accuracy: {metrics['accuracy_avg']:.2f} ± {metrics['accuracy_std']:.2f}, "
