...
def load_model_and_tokenizer(path):
    ...
    model = SLiMedNet(state_embed_dim=5, model=model)
...
def get_state_tensor(state, device='cuda'):
    state_mapping = config['emotion_mapping']
...
def generate_and_calculate_perplexity_ratio(model, tokenizer, prompt, states, num_generations=100):
    ...
    for state in states:
        ...
        ratios[state] = ratio
        print(f"Completed evaluating {state}: with: {avg_perplexity_with_state} without {avg_perplexity_without_state}\n")
    return ratios
...
def plot_perplexity_ratios(ratios):
    ...
    for index, row in relevance_increase_df.iterrows():
        plt.text(
            x=index,
            y=row["Perplexity Ratio"] + 0.02,  # Adjust the y-position slightly above the bar
    ...
    # Display the plot
    plt.savefig('resources/evaluation_results/perplexity_ratio/perplexity_ratio_emo.png')
...
def calculate_kl_divergence(logits_with_state, logits_without_state):
    ...
    kl_divergence = F.kl_div(log_probs_with_state, log_probs_without_state, reduction='batchmean', log_target=True)
...
if __name__ == "__main__":
    ...
    checkpoint = "/home/qequ54zi/thesis/submission/SLiM/resources/checkpoints/SLiM_emotions_wo_16.pth"
