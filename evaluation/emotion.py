...
def evaluate_generations_with_auc(model, tokenizer, states, num_generations=100, num_runs=5, steered=False):
    ...
    for run in range(num_runs):
        ...
        all_true_labels = []
        all_predicted_scores = {state: [] for state in states}  # Dictionary to store scores for each state
        y_pred = []
        y_true = []
        for state in states[:-1]:
            ...
            generated_texts = generate_text(model=model, tokenizer=tokenizer, prompt="I feel so", state_tensor=state_tensor, num_generations=num_generations)
            ...
            for i, classification in enumerate(classifications):
                ...
                for target_state in states[:-1]:
                    ...
                    score = next((x['score'] for x in classification if x['label'].lower() == score_state), 0)
                ...
                predicted_label = max(classification, key=lambda x: x['score'])['label'].lower()
    ...
    plt.savefig(f'resources/evaluation_results/emotion/AUC_emotion_{mode}_evaluation_new.png')

# def evaluate_unsteered_generations_with_auc(model, tokenizer, states, num_generations=100):
#     results = {state: [] for state in states}  # Store binary labels and scores per state for AUC calculation
#     all_true_labels = []
#     all_predicted_scores = {state: [] for state in states}  # Dictionary to store scores for each state
#     y_pred = []
#     y_true = []
...
#         generated_texts = generate_text(model=model, tokenizer=tokenizer, prompt="I feel so", state_tensor=None, num_generations=num_generations)
#         # Get the classification probabilities for each generated text
#         classifications = classifier(generated_texts)
...
if __name__ == "__main__":
    ...
    # evaluate_generations_with_argmax(model, tokenizer, states)