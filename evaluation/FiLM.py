...
def load_model_and_tokenizer(path):
    ...
    model = SLiMedNet(state_embed_dim=10, model=model, record_film=True, record_hidden_states=True)
...
film_values_by_state = defaultdict(lambda: {"gamma": [[] for _ in model.apply_film_at_layers],
...
hidden_states_by_state = defaultdict(lambda: {
        ...
        for key in film_values:
            for layer_idx in range(len(film_values[key])):
                ...
                if i < len(film_values[key][layer_idx]):
                    film_values_by_state[state_key][key][layer_idx].append(film_values[key][layer_idx][i])
...
scalers = {layer_idx: {"unmodulated": MinMaxScaler(), "modulated": MinMaxScaler()}
           for layer_idx in model.apply_film_at_layers}

# Plot histograms for each unique state
for state_key, values in film_values_by_state.items():
    fig, axs = plt.subplots(len(model.apply_film_at_layers), 2, figsize=(20, 2 * len(model.apply_film_at_layers)))
    ...
    for i, layer_idx in enumerate(model.apply_film_at_layers):
        ...
        if values["scale"][layer_idx].size > 0:
            axs[i, 0].hist(values["scale"][layer_idx][0,:,:].flatten(), bins=50, color='salmon', density=True)
            axs[i, 0].set_title(f'Scale Layer {layer_idx}')
            axs[i, 0].set_xlim(-1, 1)

        if values["shift"][layer_idx].size > 0:
            axs[i, 1].hist(values["shift"][layer_idx][0,:,:].flatten(), bins=50, color='lightgreen', density=True)
...
for state_key, state_hidden_states in hidden_states_by_state.items():
    if state_key == (np.int64(1), np.int64(0), np.int64(0), np.int64(0), np.int64(0)):
        for layer_idx in model.apply_film_at_layers:
            ...
            for layer_idx in model.apply_film_at_layers:
                unmodulated_state = np.concat(state_hidden_states["unmodulated"][layer_idx], axis=0)
                modulated_state = np.concat(state_hidden_states["modulated"][layer_idx], axis=0)

                # Flatten for normalization
                unmodulated_state_2d = unmodulated_state.reshape(-1, unmodulated_state.shape[-1])
                modulated_state_2d = modulated_state.reshape(-1, modulated_state.shape[-1])

                # Normalize hidden states for the specific state
                unmodulated_normalized = scalers[layer_idx]["unmodulated"].fit_transform(unmodulated_state_2d)
                modulated_normalized = scalers[layer_idx]["modulated"].fit_transform(modulated_state_2d)
                ...
                plt.title(f'Averaged Modulated vs. Unmodulated Hidden States for Layer {layer_idx}, State {state_key}')
                plt.plot([0, 1], [0, 1], color='gray', linestyle='--', linewidth=1)
                plt.grid(True)
                plt.axis('equal')
                plt.tight_layout()
                plt.savefig(f"scatter_avg_modulated_vs_unmodulated_layer_{layer_idx}_state_{state_key}.png")
                plt.show()

# from sklearn.manifold import TSNE
# import matplotlib.pyplot as plt

# # Initialize storage for concatenated scale parameters by state for t-SNE
# scale_by_state = defaultdict(lambda: [])

# # Concatenate all scale parameters for each unique state
# for state_key, values in film_values_by_state.items():
#     for layer_idx in range(len(model.apply_film_at_layers)):
#         # Flatten and store scale parameters by state and layer
#         scale_flattened = values["scale"][layer_idx].squeeze(1)
#         scale_by_state[state_key].append(scale_flattened)

# breakpoint()

# # Perform t-SNE for each layer's scale parameters
# for layer_idx in range(len(model.apply_film_at_layers)):
#     # Gather all scales across states for the current layer
#     scales_all_states = []
#     state_labels = []
    
#     for state_key, scales in scale_by_state.items():
#         if layer_idx < len(scales) and scales[layer_idx].size > 0:
#             scales_all_states.append(scales[layer_idx])
#             state_labels.extend([state_key] * len(scales[layer_idx]))
    
#     # Concatenate for t-SNE input
#     if scales_all_states:
#         scales_all_states = np.array(scales_all_states)
#         scales_all_states = scales_all_states.reshape(-1, scales_all_states.shape[-1])
        
#         # Apply t-SNE
#         tsne = TSNE(n_components=2, random_state=0, n_iter=300)
#         tsne_results = tsne.fit_transform(scales_all_states)
        
#         # Plot t-SNE results
#         plt.figure(figsize=(10, 8))
#         scatter = plt.scatter(tsne_results[:, 0], tsne_results[:, 1], c=state_labels, cmap='viridis', alpha=0.7)
#         plt.colorbar(scatter, ticks=range(len(scale_by_state)))
#         plt.title(f't-SNE of Scale Parameters for Layer {layer_idx} by State')
#         plt.xlabel("t-SNE Dimension 1")
#         plt.ylabel("t-SNE Dimension 2")
#         plt.grid(True)
#         plt.savefig(f"tsne_scale_layer_{layer_idx}.png")
#         plt.show()