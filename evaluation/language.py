...
def evaluate_generations_with_auc(model, tokenizer, states, num_generations=100, num_runs=5, steered=False):
    ...
    for run in range(num_runs):
        ...
        # label_mapping = {"LABEL_2": "positive", "LABEL_0": "negative", "LABEL_1": "neutral"}
        y_pred = []
        y_true = []
        ...
        for state in states[:-1]:
            ...
            generated_texts = generate_text(model=model, tokenizer=tokenizer, prompt="Okay,", state_tensor=state_tensor, num_generations=num_generations)
            ...
            for classification in classifications:
                ...
                for target_state in states[:-1]:
                    score = 0
                    score = classification['score'] if classification['label'] == target_state else 0
    ...
    plt.savefig(f'resources/evaluation_results/AUC/AUC_lang_{mode}_evaluation_new.png')
...
def evaluate_unsteered_generations_with_auc(model, tokenizer, states, num_generations=100):
    ...
    for state in states[:-1]:
        ...
        generated_texts = generate_text(model=model, tokenizer=tokenizer, prompt="Okay,", state_tensor=None, num_generations=num_generations)
        # Get the classification results for each generated text
        classifications = classifier(generated_texts)
    ...
    ax.set_xlabel("False Positive Rate")
    ax.set_ylabel("True Positive Rate")
    # ax.set_title("ROC Curves with AUC for Each Sentiment State")
    ax.legend(loc="lower right")
    plt.savefig('resources/evaluation_results/AUC/AUC_lang_unsteered_26.11.png')

from sklearn.metrics import precision_score, recall_score, f1_score

if __name__ == "__main__":
    print("Loading checkpoint...\n")
    checkpoint = "/home/qequ54zi/thesis/submission/SLiM/resources/checkpoints/SLiM_langauge_wo_24.pth"
